...
exec(
  `docker run --name ${containerName} -p 5432:${port} -e POSTGRES_PASSWORD=${password} -e POSTGRES_USER=${user} -e POSTGRES_DB=${dbName} -d postgres`,
  (err, stdout) => {
    if (err) {
    ...
    const checkDb = () => {
      ...
      exec(`docker exec ${containerName} psql -U postgres -c "SELECT 1" -d ${dbName}`, (err) => {
        if (err) {
